...
def armortype(img_raw, rotated_rect):
    try:
        points = np.int0(cv2.boxPoints(rotated_rect))        # 获取旋转矩形的点并创建掩码
        mask = np.zeros_like(img_raw[:, :, 0], dtype=np.uint8)
        cv2.fillConvexPoly(mask, points, 255)
        roi = cv2.bitwise_and(img_raw, img_raw, mask=mask)        # 应用掩码并获取 ROI
        x, y, w, h = cv2.boundingRect(points)
        roi = roi[y:y + h, x:x + w]
        hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)        # 转换为 HSV 并定义颜色范围
        color_ranges = {
        ...
        masks = {color: cv2.inRange(hsv, *ranges) for color, ranges in color_ranges.items()}        # 计算掩码
        total_pixels = roi.size // 3        # 计算非零像素
        if total_pixels == 0:
            return -1
        red_pixels = np.count_nonzero(masks['red']) + np.count_nonzero(masks['red_upper'])
        blue_pixels = np.count_nonzero(masks['blue']) + np.count_nonzero(masks['blue_upper'])        # 统计颜色像素
        black_pixels = np.count_nonzero(masks['black'])
        total_non_black = total_pixels - black_pixels
        if total_non_black <= 0:
            return -1
        red_ratio = red_pixels / total_non_black        # 计算比例
        blue_ratio = blue_pixels / total_non_black
        if red_ratio > blue_ratio and red_ratio > 0.5:        # 确定主导颜色
            return 0
...
def track_armor(img, img_raw, rotated_rects, angle_tol=15, height_tol=100, width_tol=100, cy_tol=100):
    ...
    for armor_rect in armor_rects:
        ...
        if class_id is not None:
            armors_dict[f"{int(center[0])}"] = {"class_id": class_id, "height": int(height), "center": [int(center[0]), int(center[1])]}
...
def detect_armor(img, val=148, gamma=0.6, color=(0,0,0)):
    ...
    return armors_dict     #print(armors_dict)
def destroy():
    cv2.waitKey(0)
